...
CEPHES_API double torch_cephes_log ( double x );
CEPHES_API double torch_cephes_polevl ( double x, void *P, int N );
CEPHES_API double torch_cephes_p1evl ( double x, void *P, int N );
#else
...
if( z >= 1.0 )
	...
	if( x == -1.0 )
		return( -torch_cephes_INFINITY );
	torch_cephes_mtherr( "atanh", DOMAIN );
...
if( z < 0.5 )
	{
	z = x * x;
	s = x   +  x * z * (torch_cephes_polevl(z, P, 4)
                            / torch_cephes_p1evl(z, Q, 5));
	return(s);
	}

return( 0.5 * torch_cephes_log((1.0+x)/(1.0-x)) );
}
