...
local function applyNotInPlace(input, output, func)
    ...
    local inputdata = input:storage() -- torch.data(input)
    local outputdata = output:storage() -- torch.data(output)
...
local function mapNotInPlace(inputA, inputB, output, func)
    ...
    local inputAdata = inputA:storage() -- torch.data(inputA)
    local inputBdata = inputB:storage() -- torch.data(inputB)
    local outputdata = output:storage() -- torch.data(output)
...
local function create_wrapper(name, parameters, returnType)
    ...
    local function wrapper(...)
        ...
        if result then
            ...
            result = cephes[name](unpack(params))
        end

        if reportError > 0 and cephes.torch_cephes_geterr() ~= 0 then
            local errString =  "Cephes error (" .. cephes.torch_cephes_geterr() .. ")"
...
for _, v in ipairs(functions_list) do
    ...
    if not skip then
        rawset(cephes, v.name, create_wrapper('torch_cephes_' .. v.name, v.arguments, v.returnType))
    end
end
